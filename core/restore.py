...
import yaml
from app.settings import GLOBAL_CONFIG_PATH, JOBS_DIR
...
def restore_files(
    ...
    # Track original job name for cleanup
    original_job_name = job_name
    
    # Define sanitized_job early to ensure it's available throughout the function
    sanitized_job = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in job_name)
    ...
    try:
        ...
        backup_set = get_backup_set_by_job_and_set(job_name, backup_set_id)
        if not backup_set:
            # Try with sanitized job name (already defined above)
            backup_set = get_backup_set_by_job_and_set(sanitized_job, backup_set_id)
        ...
        # Use the sanitized job name for config file lookup (already defined above)
        job_config_path = os.path.join(JOBS_DIR, f"{sanitized_job}.yaml")
        try:
        ...
        # We need a valid destination for tarball path reconstruction
        if 'destination' not in config:
            error_msg = "No backup destination path found in configuration"
            logger.error(error_msg)
            return {"restored": [], "errors": [{"file": "config", "error": error_msg}]}
            
        logger.debug(f"Using backup source location: {config['destination']}")

        # For restore destination, use the provided path or fail with an error
        if not dest:
            error_msg = "No restore destination specified"
            logger.error(error_msg)
            return {"restored": [], "errors": [{"file": "destination", "error": error_msg}]}
            
        # Validate the destination directory exists and is writable
        if not os.path.exists(dest):
            error_msg = f"Destination directory does not exist: {dest}"
            logger.error(error_msg)
            return {"restored": [], "errors": [{"file": "destination", "error": error_msg}]}
        
        if not os.access(dest, os.W_OK):
            error_msg = f"Destination directory is not writable: {dest}"
            logger.error(error_msg)
            return {"restored": [], "errors": [{"file": "destination", "error": error_msg}]}
...
def restore_full(job_name: str, backup_set_id: str, dest: Optional[str] = None, base_dir: Optional[str] = None, event_id: Optional[str] = None) -> Dict[str, List]:
    ...
    try:
        ...
        # If dest is None, use the source_path directly from the database row
        if dest is None:
            try:
                source_path = backup_set['source_path'] if 'source_path' in backup_set.keys() else None
                if source_path:
                    dest = source_path
                    logger.info(f"Using source path from database: {dest}")
                else:
                    error_msg = "No source path found in backup set record"
                    logger.error(error_msg)
                    return {"restored": [], "errors": [{"file": "destination", "error": error_msg}]}
            except Exception as e:
                logger.error(f"Error accessing source_path from backup set: {e}")
                return {"restored": [], "errors": [{"file": "database", "error": str(e)}]}
        ...
        # Use the simplified restore_files function with only the parameters it accepts
        result = restore_files(
            original_job_name, backup_set_id, files_to_restore, dest=dest, logger=logger
        )
